...
class TimeSeqMoSEMERTModel(lightning.LightningModule):
    def __init__(
        ...
        self.time_att_cf = nn.ModuleList()
        for _ in range(self.num_cf):
            self.time_att_cf.append(AvgPoolAtt1D(time_length, config["random_att"]))
    ...
    def _forward(self, batch):
        ...
        for i in range(self.num_cf):
            att_cf = self.time_att_cf[i](mert_feature)

            if self.phase == "att":
                att_cf_list.append(att_cf)
                continue
        ...
        if self.phase == "att":
            if self.num_cf > 0:
                return att, torch.stack(att_cf_list)
            return att, None
        elif self.phase == "rep":
            if self.num_cf > 0:
                return x_rep, torch.stack(x_cf_rep_list)
            return x_rep, None
        elif self.phase in ("predict", "test"):
            if self.num_cf > 0:
                return y_pred, torch.stack(y_cf_list)
            return y_pred, None
