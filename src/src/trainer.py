...
def test_model(
    ...
    y_pred = torch.unflatten(y_pred, dim=0, sizes=(1, y_pred.shape[0]))
...
def aggregate_pred(list_pred):
    ...
    if y_cf_:
        return torch.cat(y_pred_, dim=0), torch.cat(y_cf_, dim=1)
    return torch.cat(y_pred_, dim=0), None

def write_test_result(y_pred, data_module, base_filename):
